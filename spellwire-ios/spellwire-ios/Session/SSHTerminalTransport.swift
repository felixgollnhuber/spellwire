...
nonisolated final class SSHTerminalTransport: TerminalTransport, @unchecked Sendable {
    ...
    private func connectSync() {
        guard !password.isEmpty else {
            notifyDisconnect(error: TransportError.missingPassword)
            return
        }
...
private struct SimplePasswordDelegate: NIOSSHClientUserAuthenticationDelegate {
    let username: String
    let password: String
